...
ggsave("output/figures/HUB-mean-obs-location.png", width = 9, height = 4)
...
ggsave("output/figures/HUB-average-scores.png", width = 7, height = 3.5)
```
...
# make two separate plots to keep ordering. I.e. order locations according 
# to log scale to illustrate that the ordering changes for cases

get_order <- function(df, type = "Cases") {
  df |> 
    filter(scale == "natural", target_type == type) |>
...
order_cases <- get_order(plot_df)
order_deaths <- get_order(plot_df, type = "Deaths")

plot <- function(plot_df, order) {
  plot_df |>
    mutate(location = factor(location, levels = order)) |>
    ggplot(aes(y = interval_score, x = location)) + 
    geom_bar(stat = "identity") + 
    scale_fill_brewer(palette = "Set1", name = "Forecast target") + 
...
cases <- plot_df |>
  ...
  plot(order = order_cases)

deaths <- plot_df |>
  filter(target_type == "Deaths") |>
  plot(order = order_deaths)
