...
ggsave("output/figures/HUB-mean-obs-location.png", width = 9, height = 8)
...
hub_data |>
  ...
  geom_function(fun = function(x) x^2, linetype = "dashed", size = 0.5) + 
  theme_scoringutils() + 
  geom_smooth(alpha = 0.2, size = 0.2)

ggsave("output/figures/HUB-mean-var-obs.png", width = 7, height = 4)
...
## Actual model rankings for single forecasts
...
rankings <- scores |>
  select(-c(coverage_deviation, bias, aem, sharpness, underprediction, overprediction)) |>
  group_by(location, forecast_date, horizon, target_type, scale) |>
  mutate(ranking = rank(interval_score) / n()) |>
   filter(forecast_date == "2021-10-18", 
         target_type == "Cases", 
         location == "AT") 

factor_levels <-  rankings |>
  filter(horizon == 2, scale == "natural") |>
  group_by(model) |>
...
rankings_diff <- rankings |>
  select(model, location, target_end_date, forecast_date, interval_score, scale, ranking) |>
  pivot_wider(names_from = scale, values_from = ranking, id_cols = -interval_score)

p_diff_ranking <- rankings_diff |>
  filter(horizon == 2) |>
...
p_rankings + p_diff_ranking + 
  plot_layout(widths = c(2, 1))

ggsave("output/figures/HUB-ranking-diffs.png", width = 9, height = 5)
...
ggsave(filename = "output/figures/HUB-model-ICM.png", width = 7, height = 4.5, plot = ICM)
ggsave(filename = "output/figures/HUB-model-epiBATS.png", width = 7, height = 4.5, plot = epiBATS)
ggsave(filename = "output/figures/HUB-model-BSLCoV.png", width = 7, height = 4.5, plot = BSLCoV)
...
## Variance of scores across locations 
