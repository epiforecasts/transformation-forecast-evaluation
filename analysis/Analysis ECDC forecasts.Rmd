...
box_plot_obs <- hub_data |>
  ...
  labs(y = "Observations", x = "Target type")
...
plot_mean_scores <- plot_df |>
  ...
  scale_x_discrete(guide = guide_axis(n.dodge=2), labels = label_fn_within) +
  labs(y = "Mean interval score", x = "Location") 
...
box_plot_horizon <- scores |> 
  ...
  theme(legend.position = "none") + 
  scale_y_continuous(labels = scale_fn, trans = "log10") +
  labs(y = "Rel. increase in WIS", x = "Forecast horizon (weeks)")
...
plot_means_obs  + box_plot_obs + plot_mean_scores + box_plot_scores + box_plot_horizon +
...
ggsave("output/figures/HUB-correlations.png", width = 7, height = 6)
...
plot_ranking_change <- summarised_pairwise |>
  ...
  ggplot(aes(y = rank, x = x, group = model, label = model)) +
    geom_path(aes(x=x_arrow, color = color), arrow = arrow(length = unit(0.02,"npc")), 
...
ensemble_cases <- plot_pred_score(hub_data |> filter(location == "DE"), 
                                  scores, "EuroCOVIDhub-ensemble", 
                                  remove_x_text = TRUE, 
                                  type = "Cases")

ensemble_deaths <- plot_pred_score(hub_data |> filter(location == "DE"), 
                                   scores, "EuroCOVIDhub-ensemble", 
                                   type = "Deaths")

put_plot_together <- function(plot_cases, 
                              plot_deaths) {
  layout <- "
AE
BF
...
put_plot_together(ensemble_cases, ensemble_deaths)
ggsave(filename = "output/figures/HUB-model-comparison-ensemble.png", width = 9, height = 7)


# baseline <- plot_pred_score(hub_data |> filter(location == "DE"), scores, "EuroCOVIDhub-baseline") 
# epinow <- plot_pred_score(hub_data |> filter(location == "DE"), scores, "epiforecasts-EpiNow2") 
# 
# ensemble / epinow +
#   plot_layout(guides = "collect", 
#               heights = c(1, 1, 2)) +
#   theme(legend.position = "bottom")
# 
# ggsave(filename = "output/figures/HUB-model-comparison.png", width = 9, height = 7)
