...
%DIF DEL second submission after review//Manuscript-without-SI.tex   Mon Jul 10 11:39:59 2023
%DIF ADD final submission after reformatting/final-manuscript.tex    Tue Aug 15 16:56:38 2023
%DIF < \documentclass{article}
%DIF < \usepackage[]{graphicx}
%DIF < \usepackage[]{xcolor}
%DIF < \usepackage{alltt}
%DIF < \usepackage[left=2.3cm,right=2.8cm, top = 2.2cm, bottom = 3cm]{geometry}
%DIF < \usepackage{amsmath}
%DIF < \usepackage{amssymb}
%DIF < \usepackage{natbib}
%DIF < \PassOptionsToPackage{hyphens}{url}
%DIF < \usepackage{url} 
%DIF < \usepackage[disable]{todonotes}
%DIF < \usepackage{multicol}
%DIF < \usepackage{rotating}
%DIF -------
% Template for PLoS %DIF > 
% Version 3.6 Aug 2022 %DIF > 
% %DIF > 
% % % % % % % % % % % % % % % % % % % % % % %DIF > 
% %DIF > 
% -- IMPORTANT NOTE %DIF > 
...
% %DIF > 
% %DIF > 
% There are no restrictions on package use within the LaTeX files except that no packages listed in the template may be deleted. %DIF > 
% %DIF > 
...
% %DIF > 
% For inline equations, please be sure to include all portions of an equation in the math environment.  For example, x$^2$ is incorrect; this should be formatted as $x^2$ (or $\mathrm{x}^2$ if the romanized font is desired). %DIF > 
% %DIF > 
% Do not include text that is not math in the math environment. For example, CO2 should be written as CO\textsubscript{2} instead of CO$_2$. %DIF > 
% %DIF > 
...
% %DIF > 
% When adding superscript or subscripts outside of brackets/braces, please group using {}.  For example, change "[U(D,E,\gamma)]^2" to "{[U(D,E,\gamma)]}^2".  %DIF > 
% %DIF > 
...
\usepackage[nopatch=eqnum]{microtype} %DIF > 
...
\usepackage{booktabs}
%DIF 15-21c173-189
%DIF < \usepackage[colorlinks=false]{hyperref} 
%DIF < \setlength{\parskip}{\baselineskip}%
...
\begin{document}
\DIFaddbegin \vspace*{0.2in}
\DIFaddend 

\DIFdelbegin %DIFDELCMD < \title{Scoring epidemiological forecasts on transformed scales}
%DIFDELCMD <   \author{Nikos I. Bosse\thanks{Department of Infectious Disease Epidemiology, London School of Hygiene \& Tropical Medicine, London, United Kingdom} $^{,}$\thanks{Centre for the Mathematical Modelling of Infectious Diseases, London, United Kingdom} $^{,}$\thanks{NIHR Health Protection Research Unit in Modelling \& Health Economics} $^{,*}$,  
%DIFDELCMD <   Sam Abbott\footnotemark[1] $^{,}$\footnotemark[2]$ ^{}$, 
%DIFDELCMD <   Anne Cori\thanks{MRC Centre for Outbreak Analysis and Modelling, Department of Infectious Disease Epidemiology, School of Public Health, Imperial College London, London, United Kingdom} $^{}$, \\
%DIFDELCMD <   Edwin van Leeuwen\footnotemark[1] $^{,}$\thanks{Modelling \& Economics Unit and NIHR Health Protection Research Unit in Modelling \& Health Economics, UK Health Security Agency, London, United Kingdom} $^{}$, 
%DIFDELCMD <   Johannes Bracher\thanks{Chair of Statistical Methods and Econometrics, Karlsruhe Institute of Technology, Karlsruhe, Germany } $^{,}$\thanks{Computational Statistics Group, Heidelberg Institute for Theoretical Studies, Heidelberg, Germany } $^{, \dagger}$, 
%DIFDELCMD <   Sebastian Funk\footnotemark[1] $^{, }$\footnotemark[2] $^{, }$\footnotemark[3] $ ^{, \dagger}$}
%DIFDELCMD < %%%
\DIFdelend %DIF >  Title must be 250 characters or less.
\DIFaddbegin \begin{flushleft}
{\Large
\textbf\newline{Scoring epidemiological forecasts on transformed scales} %DIF >  Please use "sentence case" for title and headings (capitalize only the first word in a title (or heading), the first word in a subtitle (or subheading), and any proper nouns).
}
\newline
%DIF >  Insert author names, affiliations and corresponding author email (do not include titles, positions, or degrees).
\\
\DIFadd{Nikos I. Bosse\textsuperscript{1, 2, 3, *},
...
\bigskip
\textbf{\DIFadd{1}} \DIFadd{Department of Infectious Disease Epidemiology, London School of Hygiene \& Tropical Medicine, London, United Kingdom
}\\
\textbf{\DIFadd{2}} \DIFadd{Centre for the Mathematical Modelling of Infectious Diseases, London, United Kingdom
}\\
\textbf{\DIFadd{3}} \DIFadd{NIHR Health Protection Research Unit in Modelling \& Health Economics
}\\
\textbf{\DIFadd{4}} \DIFadd{MRC Centre for Outbreak Analysis and Modelling, Department of Infectious Disease Epidemiology, School of Public Health, Imperial College London, London, United Kingdom
}\\
\textbf{\DIFadd{5}} \DIFadd{Modelling \& Economics Unit and NIHR Health Protection Research Unit in Modelling \& Health Economics, UK Health Security Agency, London, United Kingdom
}\\
\textbf{\DIFadd{6}} \DIFadd{Chair of Statistical Methods and Econometrics, Karlsruhe Institute of Technology, Karlsruhe, Germany
}\\
\textbf{\DIFadd{7}} \DIFadd{Computational Statistics Group, Heidelberg Institute for Theoretical Studies, Heidelberg, Germany
}\\
\bigskip
\DIFaddend 
...
%DIFDELCMD < %%%
\DIFdelend %DIF >  Insert additional author notes using the symbols described below. Insert symbol callouts after author names as necessary.
%DIF >  
%DIF >  Remove or comment out the author notes below if they aren't used.
%DIF > 
%DIF >  Primary Equal Contribution Note
\DIFaddbegin \Yinyang \DIFadd{These authors contributed equally to this work.
}\DIFaddend 

%DIF >  Use the asterisk to denote corresponding authorship and provide email address in note below.
\DIFaddbegin \DIFadd{* nikos.bosse@lshtm.ac.uk
}
...
\DIFdel{$^*$ Correspondence to }%DIFDELCMD < \url{nikos.bosse@lshtm.ac.uk}}%%%
\DIFdel{, 
}%DIFDELCMD < {\footnotesize %%%
\DIFdel{$^\dagger$ Contributed equally}%DIFDELCMD < }
%DIFDELCMD < 
...
\DIFdelbegin \DIFdel{Probabilistic forecasts \mbox{%DIFAUXCMD
\citep{heldProbabilisticForecastingInfectious2017} }\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \section*{\DIFadd{Introduction}}

\DIFadd{Probabilistic forecasts \mbox{%DIFAUXCMD
\cite{heldProbabilisticForecastingInfectious2017} }\hskip0pt%DIFAUXCMD
}\DIFaddend play an important role in decision-making in epidemiology and public health \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{doi:10.2105/AJPH.2022.306831}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{doi:10.2105/AJPH.2022.306831}}\hskip0pt%DIFAUXCMD
}\DIFaddend , as well as other areas as diverse as economics \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{timmermannForecastingMethodsFinance2018} }\hskip0pt%DIFAUXCMD
or meteorology \mbox{%DIFAUXCMD
\citep{gneitingWeatherForecastingEnsemble2005}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{timmermannForecastingMethodsFinance2018} }\hskip0pt%DIFAUXCMD
or meteorology \mbox{%DIFAUXCMD
\cite{gneitingWeatherForecastingEnsemble2005}}\hskip0pt%DIFAUXCMD
}\DIFaddend . Forecasts based on epidemiological modelling in particular have received widespread attention during the COVID-19 pandemic. Evaluations of forecasts can provide feedback for researchers to improve their models and train ensembles. They moreover help decision-makers distinguish good from bad predictions and choose forecasters and models that are best suited to inform future decisions.

Probabilistic forecasts are usually evaluated using so-called (strictly) proper scoring rules \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{gneitingStrictlyProperScoring2007}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{gneitingStrictlyProperScoring2007}}\hskip0pt%DIFAUXCMD
}\DIFaddend , which return a numerical score as a function of the forecast and the observed data. 
...
Common proper scoring rules are the logarithmic or log score \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{goodRationalDecisions1952} }\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{goodRationalDecisions1952} }\hskip0pt%DIFAUXCMD
}\DIFaddend and the continuous ranked probability score \DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
\citep[CRPS,][]{gneitingStrictlyProperScoring2007}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{(CRPS,~\mbox{%DIFAUXCMD
\cite{gneitingStrictlyProperScoring2007}}\hskip0pt%DIFAUXCMD
)}\DIFaddend . The log score is the predictive log density or probability mass evaluated at the observed value. It is supported by the likelihood principle \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{Winkler1996} }\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{winklerScoringRulesEvaluation1996} }\hskip0pt%DIFAUXCMD
}\DIFaddend and has many desirable theoretical properties; however, the particularly severe penalties it assigns to occasional misguided forecasts make it little robust \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
...
}\DIFaddend . Moreover, it is not easily applied to forecasts reported as samples or quantiles, as used in many recent disease forecasting efforts. It is nonetheless occasionally used in epidemiology (see e.g., \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citealt{heldProbabilisticForecastingInfectious2017, Johansson2019}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{heldProbabilisticForecastingInfectious2017, Johansson2019}}\hskip0pt%DIFAUXCMD
}\DIFaddend ), but in recent years the CRPS and the weighted interval score \DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
\citep[WIS,][]{bracherEvaluatingEpidemicForecasts2021} }\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{(WIS,~\mbox{%DIFAUXCMD
\cite{bracherEvaluatingEpidemicForecasts2021}}\hskip0pt%DIFAUXCMD
) }\DIFaddend have become increasingly popular. 
...
\DIFaddend where $y$ is the true observed value, $F$ is the cumulative distribution function (CDF) of the predictive distribution, and $\boldsymbol{1}()$ is the indicator function. The CRPS can be understood as a generalisation of the absolute error to predictive distributions, and interpreted on the natural scale of the data. The WIS is an approximation of the CRPS for predictive distributions represented by a set of predictive quantiles and is currently used to assess forecasts in the so-called COVID-19 Forecast Hubs in the US \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{cramerCOVID19ForecastHub2020, cramerEvaluationIndividualEnsemble2021}}\hskip0pt%DIFAUXCMD
, Europe \mbox{%DIFAUXCMD
\citep{sherrattPredictivePerformanceMultimodel2022} }\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{cramerCOVID19ForecastHub2020, cramerEvaluationIndividualEnsemble2021}}\hskip0pt%DIFAUXCMD
, Europe \mbox{%DIFAUXCMD
\cite{sherrattPredictivePerformanceMultimodel2022} }\hskip0pt%DIFAUXCMD
}\DIFaddend and Germany and Poland \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{bracherShorttermForecastingCOVID192021, bracherNationalSubnationalShortterm2021}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{bracherShorttermForecastingCOVID192021, bracherNationalSubnationalShortterm2021}}\hskip0pt%DIFAUXCMD
}\DIFaddend , as well as the US \textit{FluSight} project on influenza forecasting\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{CdcepiFlusightforecastdata2022}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{CdcepiFlusightforecastdata2022}}\hskip0pt%DIFAUXCMD
...
Our analysis mostly focuses on the CRPS (or WIS) as an evaluation metric for probabilistic forecasts, given its widespread use throughout the COVID-19 pandemic. We note that the logarithmic score has scale invariance properties which imply that score differences between different forecasts are invariant to strictly monotonic transformations (see \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citealt{Lehmann1950} }\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Lehmann1950} }\hskip0pt%DIFAUXCMD
}\DIFaddend on corresponding properties of likelihood ratios and \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citealt{diksLikelihoodbasedScoringRules2011}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{diksLikelihoodbasedScoringRules2011}}\hskip0pt%DIFAUXCMD
}\DIFaddend ). The question of the right scale to evaluate forecasts on does therefore not arise for the log score. 

The remainder of the article is structured as follows. \DIFdelbegin \DIFdel{In Sections \ref{sec:methods:relative}--\ref{sec:methods:vst} }\DIFdelend \DIFaddbegin \DIFadd{First, }\DIFaddend we provide some mathematical intuition on applying the log-transformation prior to evaluating the CRPS, highlighting the connections to relative error measures, the epidemic growth rate and variance stabilizing transformations.
We then discuss the effect of the log-transformation on forecast rankings \DIFdelbegin \DIFdel{(Section \ref{sec:methods:rankings}) }\DIFdelend as well as practical considerations for applying transformations in general and the log-transformation in particular\DIFdelbegin \DIFdel{(Section \ref{sec:methods:considerations})}\DIFdelend . To analyse the real-world implications of the log-transformation we use forecasts submitted to the European COVID-19 Forecast Hub  \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep[ Section \ref{sec:HUB}]{europeancovid-19forecasthubEuropeanCovid19Forecast2021, sherrattPredictivePerformanceMultimodel2022}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{europeancovid-19forecasthubEuropeanCovid19Forecast2021,sherrattPredictivePerformanceMultimodel2022}}\hskip0pt%DIFAUXCMD
}\DIFaddend . Finally, we provide scoring recommendations, discuss alternative transformations that may be useful in different contexts, and suggest further research avenues\DIFdelbegin \DIFdel{(Section \ref{sec:discussion}}\DIFdelend ). 


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\DIFdelbegin \section{\DIFdel{Logarithmic transformation of forecasts and observations}}
%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \section*{\DIFadd{Logarithmic transformation of forecasts and observations}}
\DIFaddend \label{sec:methods}

\DIFdelbegin \subsection{\DIFdel{Interpretation as a relative error}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsection*{\DIFadd{Interpretation as a relative error}}
\DIFaddend \label{sec:methods:relative}
...
\end{equation}
\end{linenomath*}
The absolute error after log transforming is thus an approximation of the \textit{absolute percentage error} \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep[APE,][]{gneitingMakingEvaluatingPoint2011a} }\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{(APE,~ \mbox{%DIFAUXCMD
\cite{gneitingMakingEvaluatingPoint2011a}}\hskip0pt%DIFAUXCMD
) }\DIFaddend as long as forecast and observation are close. As we assumed that $\hat{y} \approx y$, we can also interpret it as an approximation of the \textit{relative error} \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep[RE,][]{gneitingMakingEvaluatingPoint2011a}
}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{(RE,~\mbox{%DIFAUXCMD
\cite{gneitingMakingEvaluatingPoint2011a}}\hskip0pt%DIFAUXCMD
)
...
\end{equation}
\end{linenomath*}
and the \textit{symmetric absolute percentage error} (SAPE; see e.g., \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citealt{Flores1986}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Flores1986}}\hskip0pt%DIFAUXCMD
}\DIFaddend )
\begin{linenomath*}
\begin{equation}
...
\DIFdelbegin \subsection{\DIFdel{Interpretation as scoring the exponential growth rate}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsection*{\DIFadd{Interpretation as scoring the exponential growth rate}}
\DIFaddend \label{sec:methods:growthrate}

Another interpretation for the log-transform is possible if the generative process is framed as exponential with a time-varying growth rate $r(t)$ \DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
\citep[see, e.g.,][]{wallingaHowGenerationIntervals2007}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{(see e.g. \mbox{%DIFAUXCMD
\cite{wallingaHowGenerationIntervals2007}}\hskip0pt%DIFAUXCMD
)}\DIFaddend , i.e.
\begin{linenomath*}
\begin{equation}
...
\DIFdelbegin \subsection{\DIFdel{Interpretation as a variance-stabilising transformation}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsection*{\DIFadd{Interpretation as a variance-stabilising transformation}}
\DIFaddend \label{sec:methods:vst}
...
If the predictive distribution for the quantity $Y$ equals the true data-generating process $F$ (an ideal forecast), the expected CRPS is given by \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{gneitingStrictlyProperScoring2007}
}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
...
}\DIFdelend \DIFaddbegin \DIFadd{(VST, \mbox{%DIFAUXCMD
\cite{bartlettSquareRootTransformation1936, dunnGeneralizedLinearModels2018}}\hskip0pt%DIFAUXCMD
...
\end{equation}
\end{linenomath*}
As $\sigma$ and $\mu$ are linked through the quadratic mean-variance relationship (or linear mean-standard deviation relationship, $\sigma = \sqrt{c} \times \mu$), the expected CRPS thus stays constant regardless of the expected value of the data-generating distribution $\mu$. The assumption of a quadratic mean-variance relationship is closely linked to the aspects discussed \DIFdelbegin \DIFdel{in Sections \ref{sec:methods:relative} and \ref{sec:methods:growthrate}}\DIFdelend \DIFaddbegin \DIFadd{earlier}\DIFaddend . It implies that relative errors have constant variance and can thus be meaningfully compared across different targets. Also, it arises naturally if we assume that our capacity to predict the epidemic growth rate does not depend on the expected outcome, i.e. does not depend on the current phase of the epidemic or the order of magnitude of current observations.

If the mean-variance relationship is linear with $\sigma^2 = c \times \mu$, as with a Poisson-distributed variable, the square root is known to be a VST \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
...
\begin{figure}[h!]
\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width = 1\textwidth]{../output/figures/illustration-effect-log-ranking-crps.png}
%DIFDELCMD < %%%
...
One common approach to this end is to add a small positive quantity, such as $a = 1$, to all observations and predictions before taking the logarithm \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{bellegoDealingLogsZeros2022}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{bellegoDealingLogsZeros2022}}\hskip0pt%DIFAUXCMD
...
A related issue occurs when the predictive distribution has a large probability mass on zero (or on very small values), as this can translate into an excessively wide forecast in relative terms. In our applied example this is illustrated in \DIFdelbegin \DIFdel{Figure \ref{fig:HUB-model-comparison-baseline}}\DIFdelend \DIFaddbegin \nameref{fig:HUB-model-comparison-baseline}\DIFaddend . In such instances, the dispersion component of the WIS is inflated for scores obtained after applying the natural logarithm because forecasts contained zero in its prediction intervals. To deal with this issue one could choose to use a higher $a$ value when applying a transformation $\log(x + a)$, for example $a = 10$ instead of the $a = 1$ that we chose to use.
...
When applying a transformation, the order of the operations matters, and applying a transformation after scores have been computed generally does not guarantee that the score remains proper. In the case of log transforms, taking the logarithm of the CRPS values, rather than scoring the log-transformed forecasts and data, results in an improper score. We illustrate this point using simulated data in \DIFdelbegin \DIFdel{Figure \ref{fig:log-improper}}\DIFdelend \DIFaddbegin \nameref{fig:log-improper}\DIFaddend , where it can be seen that in the example overconfident models perform best in terms of the log WIS. We note that strictly speaking, re-scaling average scores by the average score of a baseline model or average scores across different models  to obtain skill scores likewise leads to improper scores \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{gneitingStrictlyProperScoring2007}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{gneitingStrictlyProperScoring2007}}\hskip0pt%DIFAUXCMD
}\DIFaddend . The application of such skill scores, however, is established practice and considered largely unproblematic.

We note that in the practical evaluation of operational forecasting systems  several additional challenges arise, which we do not study in detail. These concern e.g., the removal of outlying observations and forecasts and the handling of missing forecasts. The solutions we employed in practice are \DIFdelbegin \DIFdel{provided in Section \ref{sec:HUB-setting}}\DIFdelend \DIFaddbegin \DIFadd{detailed below}\DIFaddend .

\DIFdelbegin \section{\DIFdel{Empirical example: the European Forecast Hub}}
%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \section*{\DIFadd{Empirical example: the European Forecast Hub}}
\DIFaddend \label{sec:HUB}

\DIFdelbegin \subsection{\DIFdel{Setting}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsection*{\DIFadd{Setting}}
\DIFaddend \label{sec:HUB-setting}

As an empirical comparison of evaluating forecasts on the natural and on the log scale, we use forecasts from the European Forecast Hub \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{europeancovid-19forecasthubEuropeanCovid19Forecast2021, sherrattPredictivePerformanceMultimodel2022}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{europeancovid-19forecasthubEuropeanCovid19Forecast2021, sherrattPredictivePerformanceMultimodel2022}}\hskip0pt%DIFAUXCMD
}\DIFaddend . 
The European COVID-19 Forecast Hub is one of several COVID-19 Forecast Hubs \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{cramerEvaluationIndividualEnsemble2021, bracherShorttermForecastingCOVID192021} }\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{cramerEvaluationIndividualEnsemble2021, bracherShorttermForecastingCOVID192021} }\hskip0pt%DIFAUXCMD
...
The forecasts used for the purpose of this illustration are forecasts submitted between the 8th of March 2021 and the 5th of December 2022 for reported cases and deaths from COVID-19. Target dates range from the 13th of March 2021 to the 10th of December 2022, for a total of 92 weeks. See \cite{sherrattPredictivePerformanceMultimodel2022} for a more thorough description of the data. We filtered all forecasts submitted to the Hub to only include the seven models which have submitted forecasts for both deaths and cases for 4 horizons in 32 locations on at least 46 forecast dates (see \DIFdelbegin \DIFdel{Figure \ref{fig:HUB-num-avail-models}}\DIFdelend \DIFaddbegin \nameref{fig:HUB-num-avail-models}\DIFaddend ). We removed all observations marked as data anomalies by the European Forecast Hub \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{sherrattPredictivePerformanceMultimodel2022} }\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{sherrattPredictivePerformanceMultimodel2022} }\hskip0pt%DIFAUXCMD
}\DIFaddend as well as all remaining negative observed values. These anomalies made up a relevant fraction of all observations. On average across locations, 12.1 out of 92 (13.2\%) observations were removed for cases and 12.4 out of 92 (13.5\%) for deaths. \DIFdelbegin \DIFdel{Figure \ref{fig:number-anomalies} }\DIFdelend \DIFaddbegin \nameref{fig:number-anomalies} \DIFaddend displays the number of anomalies removed for each location. In addition, we filtered out a small number of erroneous forecasts that were in extremely poor agreement with the observed data, as defined by any of the conditions listed in \DIFdelbegin \DIFdel{Table \ref{tab:erroneous}. Figure \ref{fig:erroneous-forecasts} }\DIFdelend \DIFaddbegin \nameref{tab:erroneous}\DIFadd{. }\nameref{fig:erroneous-forecasts} \DIFaddend shows the percentage of forecasts removed for each model. Those few (less than 0.2\%  of forecasts for each model) erroneous outlier forecasts had excessive influence on average scores and relative skill scores in a way that was not representative of normal model behaviour. We removed them here in order to better illustrate the effects of the log-transformation on scores that one would expect in a well-behaved scenario. In a regular forecast evaluation such erroneous forecasts should usually not be removed and would count towards overall model scores. 

All predictive quantiles were truncated at 0. We applied the log-transformation after adding a constant $a = 1$ to all predictions and observed values. The choice of $a = 1$ in part reflects convention, but also represents a suitable choice as it avoids giving excessive weight to forecasts close to zero, while at the same time ensuring that scores for observations $> 5$ can be interpreted reasonably. \DIFaddbegin \nameref{fig:illustration-effect-log-offset} \DIFadd{illustrates the effect of adding a small quantity before taking the logarithm. }\DIFaddend The analysis was conducted in \texttt{R} \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{R}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{R}}\hskip0pt%DIFAUXCMD
}\DIFaddend , using the \texttt{scoringutils} package \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{bosseEvaluatingForecastsScoringutils2022} }\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{bosseEvaluatingForecastsScoringutils2022} }\hskip0pt%DIFAUXCMD
}\DIFaddend for forecast evaluation. All code is available on GitHub 
...
In addition to the WIS we use pairwise comparisons \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{cramerEvaluationIndividualEnsemble2021} }\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{cramerEvaluationIndividualEnsemble2021} }\hskip0pt%DIFAUXCMD
}\DIFaddend to evaluate the relative performance of models across countries in the presence of missing forecasts. In the first step, score ratios are computed for all pairs of models by taking the set of overlapping forecasts between the two models and dividing the score of one model by the score achieved by the other model. The relative skill for a given model compared to others is then obtained by taking the geometric mean of all score ratios which involve that model. Low values are better, and the ``average'' model receives a relative skill score of 1. 


\DIFdelbegin \subsection{\DIFdel{Illustration and qualitative observations}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsection*{\DIFadd{Illustration and qualitative observations}}
\DIFaddend 

\DIFdelbegin %DIFDELCMD < \begin{figure}[h!]
%DIFDELCMD <     \centering
%DIFDELCMD <     \includegraphics[width=0.99\textwidth]{../output/figures/HUB-model-comparison-ensemble.png}
%DIFDELCMD <     %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Forecasts and scores for two-week-ahead predictions from the EuroCOVIDhub-ensemble made in Germany. Missing values are due to data anomalies that were removed (see section \ref{sec:HUB-setting}. 
    ...
    %DIFAUXCMD
%DIFDELCMD < \label{fig:HUB-model-comparison-ensemble}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend When comparing examples of forecasts on the natural scale with those on the log scale (see \DIFdelbegin \DIFdel{Figures \ref{fig:HUB-model-comparison-ensemble}, \ref{fig:HUB-model-comparison-baseline}, \ref{fig:HUB-model-comparison-epinow}}\DIFdelend \DIFaddbegin \DIFadd{Fig \ref{fig:HUB-model-comparison-ensemble}, }\nameref{fig:HUB-model-comparison-baseline}\DIFadd{, }\nameref{fig:HUB-model-comparison-epinow}\DIFaddend ) a few interesting patterns emerge. Missing the peak, i.e. predicting increasing numbers while actual observations are already falling, tends to contribute a lot to overall scores on the natural scale (see forecasts during the peak in May 2022 in \DIFdelbegin \DIFdel{Figure }\DIFdelend \DIFaddbegin \DIFadd{Fig }\DIFaddend \ref{fig:HUB-model-comparison-ensemble}A, B). On the log scale, these have less of an influence, as errors are smaller in relative terms (see \ref{fig:HUB-model-comparison-ensemble}C, D). Conversely, failure to predict an upswing while numbers are still low, is less severely penalised on the natural scale (see forecasts in July 2021 and to a lesser extent in July 2022 in \DIFdelbegin \DIFdel{Figure }\DIFdelend \DIFaddbegin \DIFadd{Fig }\DIFaddend \ref{fig:HUB-model-comparison-ensemble} A, B), as overall absolute errors are low. On the log scale, missing lower inflection points tends to lead to more severe penalties (see \DIFdelbegin \DIFdel{Figure }\DIFdelend \DIFaddbegin \DIFadd{Fig }\DIFaddend \ref{fig:HUB-model-comparison-ensemble}C, D). One can also observe that on the natural scale, scores tend to track the overall level of the target quantity (compare for example forecasts for March-July with forecasts for September-October in \DIFdelbegin \DIFdel{Figure }\DIFdelend \DIFaddbegin \DIFadd{Fig }\DIFaddend \ref{fig:HUB-model-comparison-ensemble}E, F). On the log scale, scores do not exhibit this behaviour and rather increase whenever forecasts are far away from the truth in relative terms, regardless of the overall level of observations. 

\begin{figure}[h!]
    \centering
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.99\textwidth]{../output/figures/HUB-mean-obs-location.png}
%DIFDELCMD <     %%%
\DIFdelendFL \caption{\DIFdelbeginFL \DIFdelFL{Observations }\DIFdelendFL 
    \DIFaddbeginFL \DIFaddFL{Forecasts }\DIFaddendFL and scores \DIFdelbeginFL \DIFdelFL{across locations and forecast horizons }\DIFdelendFL for \DIFaddbeginFL \DIFaddFL{two-week-ahead predictions from }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{European COVID-19 Forecast Hub data}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{EuroCOVIDhub-ensemble made in Germany}\DIFaddendFL . \DIFdelbeginFL \DIFdelFL{Locations }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Missing values }\DIFaddendFL are \DIFdelbeginFL \DIFdelFL{sorted according }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{due }\DIFaddendFL to \DIFdelbeginFL \DIFdelFL{the mean observed value in }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{data anomalies }\DIFaddendFL that \DIFdelbeginFL \DIFdelFL{location}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{were removed}\DIFaddendFL . 
    A\DIFaddbeginFL \DIFaddFL{, E}\DIFaddendFL : \DIFdelbeginFL \DIFdelFL{Average (across all time points) of observed cases }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{50\% }\DIFaddendFL and \DIFdelbeginFL \DIFdelFL{deaths }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{90\% prediction intervals and observed values }\DIFaddendFL for \DIFdelbeginFL \DIFdelFL{different locations. B: Corresponding boxplot (y-axis on log-scale) of all }\DIFdelendFL cases and deaths \DIFdelbeginFL \DIFdelFL{. C: Scores for two-week-ahead forecasts from the EuroCOVIDhub-ensemble (averaged across all forecast dates) for different locations, evaluated }\DIFdelendFL on the natural scale\DIFdelbeginFL \DIFdelFL{as well as after transforming counts by adding one and applying the natural logarithm}\DIFdelendFL . \DIFdelbeginFL \DIFdelFL{D}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{B, F}\DIFaddendFL : Corresponding \DIFdelbeginFL \DIFdelFL{boxplots of all individual }\DIFdelendFL scores\DIFdelbeginFL \DIFdelFL{of the EuroCOVIDhub-ensemble for two-week-ahead predictions}\DIFdelendFL . \DIFdelbeginFL \DIFdelFL{E}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{C, G}\DIFaddendFL : \DIFdelbeginFL \DIFdelFL{Boxplots for the relative change of scores for the EuroCOVIDhub-ensemble across forecast horizons. For any given forecast date }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Forecasts }\DIFaddendFL and \DIFdelbeginFL \DIFdelFL{location, forecasts were made for four different forecast horizons, resulting in four scores. All scores were divided by }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{observations on }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{score for forecast horizon one}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{log scale}\DIFaddendFL . \DIFdelbeginFL \DIFdelFL{To enhance interpretability}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{D}\DIFaddendFL , \DIFdelbeginFL \DIFdelFL{the range of visible relative changes in }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{H: Corresponding }\DIFaddendFL scores\DIFdelbeginFL \DIFdelFL{(relative to horizon = 1) was restricted to }%DIFDELCMD < [%%%
\DIFdelFL{0.1, 10}%DIFDELCMD < ]%%%
\DIFdelendFL . 
    }
    \DIFdelbeginFL %DIFDELCMD < \label{fig:HUB-mean-locations}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \label{fig:HUB-model-comparison-ensemble}
\DIFaddendFL \end{figure}



Across the dataset, the average number of observed cases and deaths varied considerably by location and target type (see \DIFdelbegin \DIFdel{Figure }\DIFdelend \DIFaddbegin \DIFadd{Fig }\DIFaddend \ref{fig:HUB-mean-locations}A and B). On the natural scale, scores show a pattern quite similar to the observations across targets (see \DIFdelbegin \DIFdel{Figure}\DIFdelend \DIFaddbegin \DIFadd{Fig }\DIFaddend \ref{fig:HUB-mean-locations}D) and locations (see \DIFdelbegin \DIFdel{Figure}\DIFdelend \DIFaddbegin \DIFadd{Fig }\DIFaddend \ref{fig:HUB-mean-locations}C). On the log scale, scores were more evenly distributed between targets (see \DIFdelbegin \DIFdel{Figure}\DIFdelend \DIFaddbegin \DIFadd{Fig }\DIFaddend \ref{fig:HUB-mean-locations}D) and locations (see \DIFdelbegin \DIFdel{Figure}\DIFdelend \DIFaddbegin \DIFadd{Fig }\DIFaddend \ref{fig:HUB-mean-locations}C). Both on the natural scale as well on the log scale, scores increased considerably with increasing forecast horizon (see \DIFdelbegin \DIFdel{Figure }\DIFdelend \DIFaddbegin \DIFadd{Fig }\DIFaddend \ref{fig:HUB-mean-locations}E). This reflects the increasing difficulty of forecasts further into the future and, for the log scale, corresponds with our expectations \DIFdelbegin \DIFdel{from Section \ref{sec:methods:growthrate}}\DIFdelend \DIFaddbegin \DIFadd{based on the theoretical considerations detailed above}\DIFaddend . 

\DIFaddbegin \begin{figure}[h!]
    \centering
    \caption{\DIFaddFL{Observations and scores across locations and forecast horizons for the European COVID-19 Forecast Hub data. Locations are sorted according to the mean observed value in that location. 
    A: Average (across all time points) of observed cases and deaths for different locations. B: Corresponding boxplot (y-axis on log-scale) of all cases and deaths. C: Scores for two-week-ahead forecasts from the EuroCOVIDhub-ensemble (averaged across all forecast dates) for different locations, evaluated on the natural scale as well as after transforming counts by adding one and applying the natural logarithm. D: Corresponding boxplots of all individual scores of the EuroCOVIDhub-ensemble for two-week-ahead predictions. E: Boxplots for the relative change of scores for the EuroCOVIDhub-ensemble across forecast horizons. For any given forecast date and location, forecasts were made for four different forecast horizons, resulting in four scores. All scores were divided by the score for forecast horizon one. To enhance interpretability, the range of visible relative changes in scores (relative to horizon = 1) was restricted to }[\DIFaddFL{0.1, 10}]\DIFaddFL{.}}
    \label{fig:HUB-mean-locations}
\end{figure}

\DIFaddend To assess the impact of the choice of offset value $a$ we extend the display from \DIFdelbegin \DIFdel{Figure }\DIFdelend \DIFaddbegin \DIFadd{Fig }\DIFaddend \ref{fig:HUB-mean-locations}C by results obtained under different specifications. Results are shown in \DIFdelbegin \DIFdel{Figure }\DIFdelend \DIFaddbegin \DIFadd{Fig }\DIFaddend \ref{fig:HUB-log-different-offsets}, where for completeness we also added the square root transformation. \DIFdelbegin \DIFdel{As discussed in Section \ref{sec:methods:considerations}, smaller }\DIFdelend \DIFaddbegin \DIFadd{Smaller }\DIFaddend values of $a$ increase the relative weight of smaller locations in the overall evaluation. In the most extreme considered case $a = 0.001$, the smallest locations in fact receive the largest weight both for deaths and cases. For very large values (see the third row of \DIFdelbegin \DIFdel{Figure }\DIFdelend \DIFaddbegin \DIFadd{Fig }\DIFaddend \ref{fig:HUB-log-different-offsets}), the relative weights strongly resemble those of the evaluation on the natural scale. We recommend using displays of this type to get an intuition for the role different locations may play for overall evaluation results.


\begin{figure}[h!]
    \centering
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.99\textwidth]{../output/figures/HUB-scores-locations-log-variants.png}
%DIFDELCMD <     %%%
\DIFdelendFL \caption{Mean WIS in different locations for different transformations applied before scoring. Locations are sorted according to the mean observed value in that location. Shown are scores for two-week-ahead forecasts of the EuroCOVIDhub-ensemble. On the natural scale (with no transformation prior to applying the WIS), scores correlate strongly with the average number of observed values in a given location. The same is true for scores obtained after applying a square-root transformation, or after applying a log-transformation with a large offset $a$. For illustrative purposes, $a$ was chosen to be 101630 for cases and 530 for deaths, 10 times the respective median observed value. For large values of $a$, $\log(x + a)$ grows roughly linearly in $x$, meaning that we expect to observe the same patterns as in the case with no transformation. For decreasing values of $a$, we give more relative weight to scores in small locations.}
    \label{fig:HUB-log-different-offsets}
\end{figure}


\DIFdelbegin \subsection{\DIFdel{Regression analysis to determine the variance-stabilizing transformation}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsection*{\DIFadd{Regression analysis to determine the variance-stabilizing transformation}}
\DIFaddend \label{sec:HUB-regression}

As argued \DIFdelbegin \DIFdel{in Section \ref{sec:methods:vst}}\DIFdelend \DIFaddbegin \DIFadd{above}\DIFaddend , the mean-variance, or mean-CRPS, relationship determines which transformation can serve as a VST. We can analyse this relationship empirically by running a regression that explains the WIS (which approximates the CRPS) as a function of the central estimate of the predictive distribution. We ran the regression
\begin{linenomath*}
\begin{equation}
...
\DIFdelFL{Relationship between median forecasts and scores. Black dots represent WIS values for two-week ahead predictions of the EuroCOVIDhub-ensemble. Shown in red are the regression lines discussed in Section \ref{sec:HUB-regression} shown in Table \ref{tab:HUB-regression}. A: WIS for two-week-ahead predictions of the EuroCOVIDhub-ensemble against median predicted values. B: Same as A, with scores obtained after applying a square-root-transformation to the data. C: Same as A, with scores obtained after applying a log-transformation to the data.}} 
    %DIFAUXCMD
%DIFDELCMD < \label{fig:HUB-regression}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < \begin{table}
%DIFDELCMD < \centering
%DIFDELCMD < \begin{tabular}{ccrrrrrr}
%DIFDELCMD < \toprule
%DIFDELCMD < %%%
\DIFdelFL{Horizon }%DIFDELCMD < & %%%
\DIFdelFL{Target }%DIFDELCMD < & %%%
\DIFdelFL{$\alpha$ }%DIFDELCMD < & %%%
\DIFdelFL{$\beta$ }%DIFDELCMD < & %%%
\DIFdelFL{$\alpha_{\sqrt{~}}$ }%DIFDELCMD < & %%%
\DIFdelFL{$\beta_{\sqrt{~}}$ }%DIFDELCMD < & %%%
\DIFdelFL{$\alpha_{\log}$ }%DIFDELCMD < & %%%
\DIFdelFL{$\beta_{\log}$ }%DIFDELCMD < \\
%DIFDELCMD < \midrule
%DIFDELCMD < %%%
%DIF <  all & all & -1.093 & 0.963 & -0.352 & 0.201 & 0.391 & 0.001\\
%DIF <  \addlinespace
...
\DIFdelFL{-0.862 }%DIFDELCMD < & %%%
\DIFdelFL{0.876 }%DIFDELCMD < & %%%
\DIFdelFL{0.790 }%DIFDELCMD < & %%%
...
\DIFdelFL{0.162 }%DIFDELCMD < & %%%
\DIFdelFL{0.660 }%DIFDELCMD < & %%%
\DIFdelFL{-0.031}%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{3 }%DIFDELCMD < & %%%
\DIFdelFL{Cases }%DIFDELCMD < & %%%
\DIFdelFL{0.372 }%DIFDELCMD < & %%%
\DIFdelFL{0.855 }%DIFDELCMD < & %%%
\DIFdelFL{1.109 }%DIFDELCMD < & %%%
\DIFdelFL{0.238 }%DIFDELCMD < & %%%
\DIFdelFL{0.882 }%DIFDELCMD < & %%%
\DIFdelFL{-0.037}%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{4 }%DIFDELCMD < & %%%
\DIFdelFL{Cases }%DIFDELCMD < & %%%
\DIFdelFL{0.816 }%DIFDELCMD < & %%%
\DIFdelFL{0.837 }%DIFDELCMD < & %%%
\DIFdelFL{1.645 }%DIFDELCMD < & %%%
\DIFdelFL{0.296 }%DIFDELCMD < & %%%
\DIFdelFL{1.009 }%DIFDELCMD < & %%%
\DIFdelFL{-0.036}%DIFDELCMD < \\
%DIFDELCMD < \addlinespace
%DIFDELCMD < %%%
\DIFdelFL{1 }%DIFDELCMD < & %%%
\DIFdelFL{Deaths }%DIFDELCMD < & %%%
\DIFdelFL{-1.146 }%DIFDELCMD < & %%%
\DIFdelFL{0.832 }%DIFDELCMD < & %%%
\DIFdelFL{0.457 }%DIFDELCMD < & %%%
\DIFdelFL{0.048 }%DIFDELCMD < & %%%
\DIFdelFL{0.376 }%DIFDELCMD < & %%%
\DIFdelFL{-0.035}%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{2 }%DIFDELCMD < & %%%
\DIFdelFL{Deaths }%DIFDELCMD < & %%%
\DIFdelFL{-0.981 }%DIFDELCMD < & %%%
\DIFdelFL{0.867 }%DIFDELCMD < & %%%
\DIFdelFL{0.443 }%DIFDELCMD < & %%%
\DIFdelFL{0.084 }%DIFDELCMD < & %%%
\DIFdelFL{0.416 }%DIFDELCMD < & %%%
\DIFdelFL{-0.028}%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{3 }%DIFDELCMD < & %%%
\DIFdelFL{Deaths }%DIFDELCMD < & %%%
\DIFdelFL{-0.807 }%DIFDELCMD < & %%%
\DIFdelFL{0.885 }%DIFDELCMD < & %%%
...
%DIFAUXCMD
%DIFDELCMD < \label{tab:HUB-regression}
%DIFDELCMD < \end{table}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend To check the relationship after the transformation, we ran the regressions
\begin{equation}
...
\end{equation} 
where $F_{\sqrt{\ }}$ is the predictive distribution on the square-root scale. A value of $\beta_{\log} = 0$ (or $\beta_{\sqrt{\ }} = 0$, respectively) would imply that scores are linearly independent of the median prediction after the transformation. A value smaller (larger) than 0 would imply that smaller (larger) targets lead to higher scores. As can be seen from Table \ref{tab:HUB-regression}, the results indeed indicate that small targets lead to larger average WIS when using the log transform ($\beta_{\log} < 0$), while the opposite is true for the square-root transform ($\beta_{\sqrt{\ }} > 0$). The results of the three regressions are also displayed in \DIFdelbegin \DIFdel{Figure }\DIFdelend \DIFaddbegin \DIFadd{Fig }\DIFaddend \ref{fig:HUB-regression}. In this empirical example, the log transformation thus helps (albeit not perfectly), to stabilise WIS values, and it does so more successfully than the square-root transformation. As can be seen from \DIFdelbegin \DIFdel{Figure }\DIFdelend \DIFaddbegin \DIFadd{Fig }\DIFaddend \ref{fig:HUB-regression}, the expected WIS scores for case targets with medians of 10 and 100,000 differ by more then a factor of ten for the square root transformation, but only a factor of around 2 for the logarithm.

\DIFdelbegin \subsection{\DIFdel{Impact of logarithmic transformation on model rankings}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{figure}[h!]
    \centering
    \caption{\DIFaddFL{Relationship between median forecasts and scores. Black dots represent WIS values for two-week ahead predictions of the EuroCOVIDhub-ensemble. Drawn in red are the regression lines as discussed in the main text and shown in Table \ref{tab:HUB-regression}. A: WIS for two-week-ahead predictions of the EuroCOVIDhub-ensemble against median predicted values. B: Same as A, with scores obtained after applying a square-root-transformation to the data. C: Same as A, with scores obtained after applying a log-transformation to the data.}} 
...
\toprule
\DIFaddFL{Horizon }& \DIFaddFL{Target }& \DIFaddFL{$\alpha$ }& \DIFaddFL{$\beta$ }& \DIFaddFL{$\alpha_{\sqrt{~}}$ }& \DIFaddFL{$\beta_{\sqrt{~}}$ }& \DIFaddFL{$\alpha_{\log}$ }& \DIFaddFL{$\beta_{\log}$ }\\
\midrule
%DIF >  all & all & -1.093 & 0.963 & -0.352 & 0.201 & 0.391 & 0.001\\
%DIF >  \addlinespace
%DIF >  all & Cases & 0.036 & 0.858 & 0.043 & 0.201 & 0.751 & -0.033\\
...
%DIF >  3 & all & -1.001 & 0.984 & -0.094 & 0.241 & 0.410 & 0.008\\
%DIF >  4 & all & -0.757 & 0.986 & 0.000 & 0.299 & 0.469 & 0.015\\
\DIFaddFL{1 }& \DIFaddFL{Cases }& \DIFaddFL{-0.862 }& \DIFaddFL{0.876 }& \DIFaddFL{0.790 }& \DIFaddFL{0.087 }& \DIFaddFL{0.433 }& \DIFaddFL{-0.024}\\
\DIFaddFL{2 }& \DIFaddFL{Cases }& \DIFaddFL{-0.243 }& \DIFaddFL{0.877 }& \DIFaddFL{0.959 }& \DIFaddFL{0.162 }& \DIFaddFL{0.660 }& \DIFaddFL{-0.031}\\
\DIFaddFL{3 }& \DIFaddFL{Cases }& \DIFaddFL{0.372 }& \DIFaddFL{0.855 }& \DIFaddFL{1.109 }& \DIFaddFL{0.238 }& \DIFaddFL{0.882 }& \DIFaddFL{-0.037}\\
\DIFaddFL{4 }& \DIFaddFL{Cases }& \DIFaddFL{0.816 }& \DIFaddFL{0.837 }& \DIFaddFL{1.645 }& \DIFaddFL{0.296 }& \DIFaddFL{1.009 }& \DIFaddFL{-0.036}\\
\addlinespace
\DIFaddFL{1 }& \DIFaddFL{Deaths }& \DIFaddFL{-1.146 }& \DIFaddFL{0.832 }& \DIFaddFL{0.457 }& \DIFaddFL{0.048 }& \DIFaddFL{0.376 }& \DIFaddFL{-0.035}\\
\DIFaddFL{2 }& \DIFaddFL{Deaths }& \DIFaddFL{-0.981 }& \DIFaddFL{0.867 }& \DIFaddFL{0.443 }& \DIFaddFL{0.084 }& \DIFaddFL{0.416 }& \DIFaddFL{-0.028}\\
\DIFaddFL{3 }& \DIFaddFL{Deaths }& \DIFaddFL{-0.807 }& \DIFaddFL{0.885 }& \DIFaddFL{0.349 }& \DIFaddFL{0.131 }& \DIFaddFL{0.453 }& \DIFaddFL{-0.019}\\
\DIFaddFL{4 }& \DIFaddFL{Deaths }& \DIFaddFL{-0.602 }& \DIFaddFL{0.891 }& \DIFaddFL{0.125 }& \DIFaddFL{0.194 }& \DIFaddFL{0.501 }& \DIFaddFL{-0.011}\\
\bottomrule
\end{tabular}
\caption{\DIFaddFL{Coefficients of three regressions for the effect of the magnitude of the median forecast on expected scores. The first regression was 
$\log[\text{WIS}(F, y)] = \alpha + \beta \times \log[\text{median}(F)], $ where $F$ is the predictive distribution and $y$ the observed value. The second one was 
$\text{WIS}(F_{\log}, \log y) = \alpha_{\log} + \beta_{\log} \cdot \log{(\text{median}(F))},$ where $F_{\log}$ is the predictive distribution for $\log y$. The third one was $\text{WIS}(F_{\sqrt{\ }}, \sqrt{y}) = \alpha_{\sqrt{\ }} + \beta_{\sqrt{\ }} \cdot \sqrt{(\text{median}(F))},$ where $F_{\sqrt{\ }}$ is the predictive distribution for $\sqrt{y}$.
}}
...
\DIFdelbegin %DIFDELCMD < \begin{figure}[h!]
%DIFDELCMD <     \centering
%DIFDELCMD <     \includegraphics[width=0.99\textwidth]{../output/figures/HUB-pairwise-comparisons.png}
%DIFDELCMD <     %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
...
\DIFdelend \DIFaddbegin \begin{figure}[h!]
    \centering
    \caption{\DIFaddFL{Changes in model ratings as measured by relative skill for two-week-ahead predictions for cases (top row) and deaths (bottom row). A: Relative skill scores for case forecasts from different models submitted to the European COVID-19 Forecast Hub computed on the natural scale. B: Change in rankings as determined by relative skill scores when moving from an evaluation on the natural scale to one on the logarithmic scale. Red arrows indicate that the relative skill score deteriorated when moving from the natural to the log scale, green arrows indicate they improved. C: Relative skill scores based on scores on the log scale. D: Difference in relative skill scores computed on the natural and on the logarithmic scale, ordered as in C. E: Relative contributions of the different WIS components (overprediction, underprediction, and dispersion) to overall model scores on the natural and the logarithmic scale. F, G, H, I, J, K: Analogously for deaths.}}
...
The log-transformation is only one of many transformations that may be useful and appropriate in an epidemiological context. One obvious option is to apply a population standardization to obtain incidence forecasts e.g., per 100,000 population \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{abbottEvaluatingEpidemiologicallyMotivated2022}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{abbottEvaluatingEpidemiologicallyMotivated2022}}\hskip0pt%DIFAUXCMD
}\DIFaddend . 
We suggested using the natural logarithm as a variance-stabilising transformation (VST). This is appropriate for variables that are approximately normally distributed and have a quadratic mean-variance relationship with $\sigma^2 = c \times \mu^2$ (this is e.g. approximately true for the negative binomoial distribution and large $\mu$). Alternatively, the square-root transformation can be appropriate in the case of a Poisson distributed variable \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{dunnGeneralizedLinearModels2018}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{dunnGeneralizedLinearModels2018}}\hskip0pt%DIFAUXCMD
}\DIFaddend . Other VST like the Box-Cox \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{boxAnalysisTransformations1964} }\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{boxAnalysisTransformations1964} }\hskip0pt%DIFAUXCMD
}\DIFaddend are conceivable as well.
...
This is using the approximation detailed in \DIFdelbegin \DIFdel{Section \ref{sec:methods:rankings} }\DIFdelend \DIFaddbegin \DIFadd{theoretical discussion on model rankings }\DIFaddend (see also \DIFdelbegin \DIFdel{Figure }\DIFdelend \DIFaddbegin \DIFadd{Fig }\DIFaddend \ref{fig:SIM-wis-state-size-mean}). Expected CRPS scores are shown for three different distributions once on the natural scale (top row) and once scored on the log scale (bottom row). 
...
Forecasts that were in extremely poor agreement with the observed values were removed from the analysis according to the criteria shown in \DIFdelbegin \DIFdel{Table \ref{tab:erroneous}}\DIFdelend \DIFaddbegin \nameref{tab:erroneous}\DIFaddend . 
...
A, E: 50\% and 90\% prediction intervals and observed values for cases and deaths on the natural scale from the EpiNow2 model \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{epinow2}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{epinow2}}\hskip0pt%DIFAUXCMD
...
%DIFDELCMD <  %%%
\DIFdelend \DIFaddbegin \bibliography{final-manuscript.bib}
