...
Log transforming forecasts and observations shifts the focus of the evaluation while preserving propriety of the score. Instead of scoring the magnitude of absolute errors this approach captures relative errors and is approximately equivalent to scoring a forecast for the growth rate of the observed quantity. 

We focus on the log transformation, as it represents a convenient way of achieving our goal of evaluating how well a forecast captures the growth dynamic inherent to epidemiological process.
...
As expected on the natural scale, scores correlate strongly (XX) with the average number of observed cases / deaths in a given country (see Fig \ref{fig:HUB-mean-locations}A, B) and this relationship is not evident on the log scale. Interestingly, there is some evidence a negative relationship between the mean number of observations in a country and score when evaluated on a log scale implying that countries with lower mean incidence may be harder to forecast.
