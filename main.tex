...
\paragraph{Variation of scores gets smaller}
The average number of observed cases and deaths varies considerably by location and target type (see Figure \ref{fig:HUB-mean-locations}A and B). On the natural scale, scores also vary considerably across targets (see Fig \ref{fig:HUB-mean-locations}D) and locations (see Fig \ref{fig:HUB-mean-locations}C). On the log scale, scores are more evenly distributed between targets (see Fig \ref{fig:HUB-mean-locations}D) and locations (see Fig \ref{fig:HUB-mean-locations}. 
...
For relative skill scores, however, rankings do change a lot. The correlation between skill scores for two week ahead forecasts on the log and natural scale is XX. The correlation is stronger for deaths than for cases and tends to be lower for longer horizons. This might be because outliers are more likely for case forecasts than for death forecasts and also are more likely with increasing forecast horizon. 

\paragraph{Deep dive}
Differences are likely driven by
- on average, under-prediction plays a larger role (in relative terms), at least for cases. 
- confidence intervals that go very low can produce artifacts --> baseline plot, not shown. 
- natural scale punishes missing the peak very harshly. Log scale also punishes when you miss a new upswing. Arguably this is very good and potentially even closer to what we want. 
...
\begin{figure}[h!]
    ...
    \caption{Scores and average observed values across 1ocations. A, Mean of observed cases and deaths across all time points for different locations. B: XXX WIS for forecasts made by the Hub ensemble. C: Distribution of interval scores for two week ahead forecasts of COVID-19 cases and deaths evaluated on the natural scale (left) and on the log scale (right).}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.9\textwidth]{output/figures/HUB-correlations.png}
...
\begin{figure}[h!]
    ...
    \caption{Changes in relative skill and the distribution of scores.Based on case forecasts only.}
    \label{fig:HUB-pairwise}
\end{figure}

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.9\textwidth]{output/figures/HUB-model-comparison.png}
