...
\end{equation*} 
where $F_{\sqrt{\ }}$ is the predictive distribution on the square-root scale. As can be seen from Table \ref{tab:HUB-regression}, the results indeed indicate small targets lead to larger average CRPS when using the log transform ($\beta_{\log} < 0$), while the opposite is true for the square-root transform ($\beta_{\sqrt{\ }} > 0$). The results of the three regressions are also displayed in Figure \ref{fig:HUB-regression}. It appears that the log transformation is most suitable to stabilise the WIS values in this empirical example. 
...
\caption{Comparison of expected CRPS values of an ideal forecaster and approximated values. Expected scores for three distributions as described in Section \ref{sec:methods:vst} and Figure \ref{fig:SIM-wis-state-size-mean} and approximated as described in \ref{sec:methods:vst}}
