...
\begin{linenomath*}
\begin{equation*}
    \text{WIS}(F, y) = \frac{1}{K} \times \sum_{k = 1}^{K} \times 2 \left[ 1(y \leq q_{\tau_k}) - \tau_k \right] \times ( q_{\tau_k} - y), 
\end{equation*}
\end{linenomath*}
where $q_{\tau}$ is the $\tau$ quantile of the forecast $F$, $y$ is the observed outcome, and $K$ is the number of predictive quantiles provided. 
...
Moreover, if forecasts are evaluated based on absolute rather than relative errors, whereas in reality errors scale with the level of observations, scores are strongly influenced by the overall order of magnitude of the quantity to forecast. There exist other scores that do not depend on the scale of the forecast target, such as the log score \citep{goodRationalDecisions1952}, which in addition is also invariant under monotonic transformations of the target, or a scaled version of the CRPS \citep{bolinLocalScaleInvariance2021}. These, however, may not have all of the desired properties of the CRPS and in particular cannot easily be transferred to a setting with predictive quantiles. With the regular CRPS and WIS the dependence on the scale of the forecast target makes it difficult to compare predictive performance across time and in different phases of an epidemic or to compare performance across targets that generally occur at different orders of magnitudes.
In the example of COVID-19 where case numbers underwent drastic changes over time following the initial emergence, forecast evaluation based on the absolute numbers would be dominated by performance around the peaks. The ability to correctly predict future upswings while case numbers are still low would count little towards overall model scores. Scores based on jointly evaluating the ability to predict numbers of cases, hospitalisation and deaths would be dominated by cases as they occur at largest numbers. 

In many fields it is common practice to forecast transformed quantities (see e.g. \cite{taylorEvaluatingVolatilityInterval1999} in finance, \cite{mayrLogLevelVAR2015} in macroeconomics, \cite{loweStochasticRainfallrunoffForecasting2014} in hydrology or \cite{fuglstadDoesNonstationarySpatial2015} in meteorology). The goal of the transformations is usually to improve accuracy of the predictions. In this paper, we discuss how transforming the forecasts and observations prior to applying the WIS (or CRPS) may be an appropriate way to improve the evaluation process by addressing the aforementioned issues. Many different transformations may be appropriate and useful, depending on the exact context, the desired focus of the evaluation, and specific aspects of the forecasts that forecast consumers care most about (see a broader discussion in Section \ref{sec:discussion}). For conceptual clarity and to allow for a more in-depth discussion, we focus most of our attention on the log-transformation as one example of many possible transformations, chosen for its simplicity and usefulness in an epidemiological context. We further focus on the CRPS/WIS as evaluation metric, given its widespread use throughout the COVID-19 pandemic. 
...
Our aim in transforming forecasts and observations is to shift the focus of the evaluation in a way that may be more appropriate for epidemiological forecasts, while preserving the propriety of the score. A particularly relevant transformation in the context of epidemic phenomena is the natural logarithm. Instead of a score representing the magnitude of absolute errors this approach makes it possible to obtain a score which a) measures relative error and at the same time b) provides an a measure for how well a forecast captures the exponential growth rate of the target quantity (see Section \ref{sec:methods:growthrate} and is c) independent from the mean of the observed quantity (see Section \ref{sec:methods:vst}). The interpretation of the log-transformed score as a score for a forecast of the exponential growth rate of a given quantity aligns the evaluation with our understanding of how epidemiological processes should be modeled and the kinds of errors models would typically make. 
...
In epidemiological contexts, the variance of the variable of interest usually scales with its mean.  A variance-stabilising transformation \citep[VST][]{bartlettSquareRootTransformation1936} is a transformation that serves to make the variance of a variable independent of its mean. 
...
On the log scale, the correlation is much weaker and slightly negative, at least in terms of rank correlation (Spearman correlation: CHECK AGAIN -0.33 and -0.36), implying a tendency towards larger scores in countries with lower mean incidences. 
...
\begin{tabular}{lllcc}
\toprule
Target & Quantity & Measure & natural & log\\
\midrule
Cases & Observations & mean & 20013 & 8.33\\
Cases & Observations & sd & 43256 & 2.07\\
Cases & Observations & var & 1871039511 & 4.30\\
\addlinespace
Deaths & Observations & mean & 244 & 4.01\\
Deaths & Observations & sd & 498 & 1.81\\
Deaths & Observations & var & 248306 & 3.28\\
\addlinespace
\hline
\addlinespace
Cases & WIS & mean & 4220 & 0.26\\
Cases & WIS & sd & 13978 & 0.35\\
\addlinespace
Deaths & WIS & mean & 30 & 0.21\\
Deaths & WIS & sd & 70 & 0.19\\
\bottomrule
\end{tabular}
...
\toprule
Horizon & Target & $\alpha$ & $\beta$ & $\alpha^*$ & $\beta^*$\\
\midrule
all & all & -0.556 & 0.877 & 0.579 & -0.028\\
\addlinespace
all & Cases & 1.112 & 0.710 & 1.280 & -0.101\\
all & Deaths & -0.433 & 0.762 & 0.523 & -0.044\\
\addlinespace
1 & all & -0.999 & 0.855 & 0.419 & -0.031\\
2 & all & -0.756 & 0.890 & 0.517 & -0.028\\
3 & all & -0.430 & 0.895 & 0.628 & -0.026\\
4 & all & -0.047 & 0.881 & 0.747 & -0.024\\
\addlinespace
1 & Cases & -0.065 & 0.764 & 0.768 & -0.067\\
2 & Cases & 0.676 & 0.748 & 1.104 & -0.089\\
3 & Cases & 1.414 & 0.710 & 1.432 & -0.109\\
4 & Cases & 2.173 & 0.658 & 1.712 & -0.124\\
\addlinespace
1 & Deaths & -0.726 & 0.735 & 0.429 & -0.048\\
2 & Deaths & -0.553 & 0.766 & 0.490 & -0.045\\
3 & Deaths & -0.352 & 0.782 & 0.552 & -0.041\\
4 & Deaths & -0.113 & 0.781 & 0.618 & -0.036\\
\bottomrule
\end{tabular}
\caption{Coefficients of two regressions for the effect of the magnitude of the median forecast on expected scores. The first regression was 
