...
\begin{figure}[h!]
\centering
\includegraphics[width = 0.8\textwidth]{output/figures/SAPE.png}
...
Another interpretation can be based around viewing the process to be forecast as exponential with time-varying growth rate $r(t)$ (see e.g. \cite{wallingaHowGenerationIntervals2007}), i.e.
\begin{linenomath*}
\begin{equation*}
...
& = \log\left[\exp\left(\sum_{i = 0}^{n - 1}\hat{r}_{t + i}\right) \cdot y_t\right] - \log\left[\exp\left(\sum_{i = 0}^{n - 1}r_{t + i} \right) \cdot y_t\right]\\
& =   \sum_{i = 0}^{n - 1}\hat{r}_{t + i} \ - \ \sum_{i = 0}^{n - 1}r_{t + i}.
\end{align*}
\end{linenomath*}
