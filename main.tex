...
While on the natural scale, aggregate forecasts will usually be dominated by predictions of quantities with high absolute values, the opposite may be true for scores based on log transformed predictions. Small absolute errors (e.g. predicting 9, rather than 3 deaths) may be large in relative terms if the quantity to forecast is very small. The strength of this effect depends on the exact relationship between the mean and the variance of the quantity of interest. 
...
One way to handle situations where scores are very unequal, as mentioned above, is to add a number $n$ to all forecasts and observations in order to reduce the relative weight of forecast targets with small quantities. 
