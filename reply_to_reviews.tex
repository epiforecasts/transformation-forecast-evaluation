...
Proper scoring rules are constructed such that they encourage honest forecasting and cannot be `gamed' or `cheated'. % In expectation, a forecaster receives the best score if their forecast $F$ coincides with the data-generating distribution $G$, that is if $F = G$.
...
\red not sure what to make of Box-Cox.
...
We slightly disagree with the notion that the terminology should not be overloaded. Not introducing another term for transformations of $\log_{e}(x + \varepsilon)$ makes the manuscript more readable and accessible to readers. In addition, our analyses suggest that adding a small context doesn't make much of a difference in most practical applications. We nevertheless completely agree that this should be made clear to the reader and have added the following text: 
...
% to model epidemiological processes. We agree with the reviewer that a size parameter of 0.1 is not realistic, but still think that the example is valuable as an illustration. 

\blue
Figure 3: is a geometric distribution realistic for epidemiological surveillance data?

\black
We agree that this is not a typical example we would expect to see in an epidemiological setting. It could happen in principle, but only in rather constructed cases. We still believe the example is valuable to illustrate the general point that the rankings between two forecasts can change even for a single observation. We added the two following sentences:

\indented
We note that the chosen example involving a geometric forecast distribution is somewhat constructed; for more similarly shaped pairs of forecast distributions this phenomenon will not typically occur.
...
Yes, dividing by the observed value generally disturbs propriety. This approach would correspond to weighting CRPS scores with weights depending on the observed value, and it has been shown that the resulting average scores are no longer proper; see Lerch et al (2017, Statistical Science, preprint: \url{https://arxiv.org/abs/1512.09244})
    ...
    \black Like in the related arguments in the paper we address this question for the simpler case of the aboslute error rather than CRPS. Using a shifted log transform corresponds to (in lsight variation of equation (8))
    \begin{equation}
...
% Is there a good source for the SAPE? It's not in the Gneiting paper. I found this, but not entirely convinced: https://bmcinfectdis.biomedcentral.com/articles/10.1186/s12879-017-2365-1. Wikipedia (\url{https://en.wikipedia.org/wiki/Symmetric_mean_absolute_percentage_error})also suggests Armstrong and Flores. 
...
We would expect that one could find a value for a (e.g. 10?) that would eventually yield a more or less flat line, but we haven't tested this in practice. 
...
\clearpage
\bibliography{log-or-not-v2.bib, software.bib}
